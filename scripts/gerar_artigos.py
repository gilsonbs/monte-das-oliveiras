...
def _trigger_site_rebuild() -> bool:
    """Aciona rebuild via Vercel Deploy Hook. Retorna True se bem-sucedido."""
    hook_url = os.environ.get("VERCEL_DEPLOY_HOOK_URL", "")
    if not hook_url:
        print("      [BUILD] VERCEL_DEPLOY_HOOK_URL ausente — rebuild ignorado.", file=sys.stderr)
        return False

    try:
        resp = requests.post(hook_url, timeout=15)
        if resp.status_code in (200, 201):
            print("      ✓ Rebuild Vercel acionado — o site será atualizado em ~1 min.")
            return True
        print(f"      ⚠ Deploy Hook retornou {resp.status_code}: {resp.text[:120]}", file=sys.stderr)
        return False
    except Exception as e:
        print(f"      ⚠ Erro ao acionar Vercel Deploy Hook: {e}", file=sys.stderr)
        return False
